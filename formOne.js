...
// comments in this file are very nice.
// code in this file could still very happily be broken down a bit more into, say, a function to handle equipment, one to handle languages, one to handle traits, etc
...
const formOne = async (req,res) => {
  ...
  // You're currently awaiting all of these in sequence...
  // 1. get race
  // 2. only when that is done, get class
  // 3. only when that is done, get martial
  // One way to speed this up would be to (you will never escape) Promise.all on them, so they'd run in parallel.
  let [raceData, classData, martialWeapons, simpleWeapons] = await Promise.all([
    axios.get(`https://www.dnd5eapi.co/api/races/${race}`),
    axios.get(`https://www.dnd5eapi.co/api/classes/${charClass}`),
    axios.get(`https://www.dnd5eapi.co/api/equipment-categories/martial-weapons`),
    axios.get(`https://www.dnd5eapi.co/api/equipment-categories/simple-weapons`)
  ]);
  ...
  mail[1].features = levelData.reduce((acc, feat) => [...acc,...feat.features],[]);
